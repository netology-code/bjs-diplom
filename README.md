...
-   Зачисление авторизованному пользователю на счет 500000 евро.
...
В данной функции нужно использовать метод `getStocks` класса `ApiConnector`. 
Изучите, что возвращает данный метод и как следует использовать полученную информацию для конвертации валюты в **Неткоины**.
...
7. В случае удачного добавления денег на счёт вызовите метод конвертации денег из текущей валюты в **Неткоины**. Обратите внимание, что для корректной конвертации вам потребуется получить данные из функции получения курса валют.
